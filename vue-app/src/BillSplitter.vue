<template>
  <div class="bill-splitter-app">
    ...
    <div v-if="showTipPopup" id="popup">
      <h3>Tip! 💡</h3>
      <p>To speed up splitting: if no one is selected in the <b>Split With</b> column, the item will be split with everyone.</p>
      <label>
    ...
    <div class="input-section">
      <form autocomplete="off" @submit.prevent="addPerson">
        <input
          ...
          placeholder="Add people (e.g., 'Alice Bob Charlie') then press Enter"
          class="wide-input"
          autocomplete="off"
        />
    ...
    <div class="input-section">
      <form autocomplete="off" @submit.prevent="addMenuItem">
        <input
          type="text"
          v-model="newMenuItemName"
          placeholder="Add items (e.g., 'Pizza, Wings') then press Enter"
          class="wide-input"
          autocomplete="off"
        />
    ...
    <table>
      ...
      <tr v-if="billItems.length === 0">
        <td colspan="4" style="text-align: center;">Add items to get started!</td>
      </tr>
      <tr v-for="item in billItems" :key="item.id">
        <td><input type="text" v-model="item.name" placeholder="Item Name" /></td>
        <td><input type="tel" :value="item.price" @input="event => formatPrice(item, event)" placeholder="Price" /></td>
        <td class="split-checkboxes">
    ...
    <div class="input-section">
      ...
      <input type="tel" id="total-input" :value="totalAmount" @input="formatTotal" placeholder="Total" @keydown.enter="showSummary = true" />
    </div>
...
/**
 * Calculates the sum of all item prices. It's reactive and updates automatically.
 */
const subtotal = computed(() => {
  return billItems.value.reduce((total, item) => {
...
async function handleReceiptUpload(event) {
  ...
  try {
    ...
    data.line_items.forEach(item => {
      addItemRow(item.name, item.price.toFixed(2));
    });

    if (data.total_amount) {
      totalAmount.value = data.total_amount.toFixed(2);
    }
  } catch (err) {
